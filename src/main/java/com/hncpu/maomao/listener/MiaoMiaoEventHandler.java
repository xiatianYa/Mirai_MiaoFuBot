package com.hncpu.maomao.listener;

import com.hncpu.maomao.Entry.BaiDuEntry;
import com.hncpu.maomao.common.ShareData;
import com.hncpu.maomao.message.MiaoMiaoSendMessage;
...
public class MiaoMiaoEventHandler extends SimpleListenerHost {
    public MiaoMiaoEventHandler() {
    ...
    public void onFriendMessage(@NotNull FriendMessageEvent event){
        ...
        if (message.get(1) instanceof PlainText) {
            ...
            if (Type.equals("配置信息")){
                ...
                groups.forEach(item->{
                    messageChainBuilder.append(item.toString()+"\n");
                });
                messageChainBuilder.append("======服务器列表======\n");
                addresses.forEach((key,value)->{
                    messageChainBuilder.append(key+" : "+value+"\n");
                });
    ...
    public void send(GroupMessageEvent event, String Type) {
        switch (Type) {
            case "娶群友":
                MiaoMiaoSendMessage.Marry(event);
                break;
            case "订婚":
                MiaoMiaoSendMessage.Engage(event);
                break;
            case "离婚":
                MiaoMiaoSendMessage.Divorce(event);
                break;
            case "当小三":
                MiaoMiaoSendMessage.Mistress(event);
                break;
            case "群老婆列表":
                MiaoMiaoSendMessage.getWifeList(event);
                break;
            case "喵服":
                MiaoMiaoSendMessage.getMiaoFuList(event);
                break;
            case "奶服":
                MiaoMiaoSendMessage.getNaiFuList(event);
                break;
            default:
                //未知信息
                break;
